                  ...
                  <v-row justify="center">
                    <v-col cols="10">
                      ...
                      <v-text-field
                        ...
                        prepend-icon="lock"
                        :append-icon="
                          chngpassword.show1 ? 'mdi-eye' : 'mdi-eye-off'
                        "
...
export default {
  ...
  setup() {
      ...
      if (isvalid) {
        ...
        loginapi
          .doPhonePasswordChange(
            chngpassword.phone,
            url,
            chngpassword.password,
            chngpassword.verification_code,
            isLegacyMode
          )
          .then((response) => {
            let msg = response.data.message
            if (msg === SMS_SENT) {
              chngpassword.errors = t("LoginScreen.SMS_SENT")
              chngpassword.showerr = true
              chngpassword.errType = "success"
              chngpassword.verification_code_sent = true
    ...
    const agreeRule = (value) => {
      const regexp = /^\d{9,}$/
      if (!value) {
        return t("LoginScreen.PHONE_REQUIRED")
      }
      if (!regexp.test(value)) {
        return t("LoginScreen.PHONE_VALID")
      }
      return true
    }
    ...
    const passwordrule = (value) => {
      if (!value) {
        return t("LoginScreen.PASSWORD_REQUIRED")
      }
      return true
    }
