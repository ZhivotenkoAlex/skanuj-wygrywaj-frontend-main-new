  ...
  <v-container :style="`background-color:${config.innercolor} !important`">
    <v-row class="mb-6">
      ...
      <v-col v-else cols="12">
        <v-card class="mx-auto my-12 mb-4" max-width="374">
          <v-img style="width: 100%" :src="product.picture"></v-img>
          <v-card-title class="card__title">{{ product.name }}</v-card-title>
          <v-card-text>
            <v-row align="center" class="mx-0 text-center">
              <div class="black--text">
                <v-card-text>
                  <strong
                    >{{ $t("RewardsScreen.VOUCHER_NUMBER") }}:
                    {{ purchase.id }}</strong
                  ></v-card-text
                >
                <v-card-text>{{
                  $t("RewardsScreen.SHOW_NUMBER_TO_STAFF")
                }}</v-card-text>
                <v-card-text>
                  <p>{{ $t("RewardsScreen.THANK_FOR_PARTICIPATING") }}</p>
                  <p>{{ $t("RewardsScreen.COME_AGAIN") }}</p>
                </v-card-text>
              </div>
    ...
    getPurchaseBy(purchase) {
      ...
      if (purchase.need_accept != 2) {
        r =
          ...
          purchase.price +
          " " +
          this.lang_points(purchase.price)
      }
      return r
    },
