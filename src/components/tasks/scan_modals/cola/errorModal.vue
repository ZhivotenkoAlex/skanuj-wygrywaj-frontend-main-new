    ...
    <v-dialog
      ...
      <v-card class="modelFullscreen"
            ...
            <div style="background-color: rgb(0 0 0 / 52%);margin: 20px;border-radius: 20px;">
              ...
              <div class="pt-12">
                <div>
                  <img src="../../../../assets/images/receipt/check-error.png" class="progressModalImage"/>
                </div>
              </div>

              <div class="pt-5 pb-5">
                <h4 class="modalHeader white--text">{{$t('TaskScreen.MODAL_ERROR_TITLE')}}</h4>
              </div>



              <h5 class="rewards-section white--text pt-3">{{errorMessage}}</h5>

              <div class="pl-5 pr-5 pb-1">
                <v-btn v-if="id > 0" @click="sendManualVerification" block class="mt-10 modalNotActionBtn w-100" style="max-width: 100%">
                  Wyślij do weryfikacji ręcznej
                </v-btn>
                <v-btn @click="hide" block class="mt-3 modalNotActionBtn mb-12 mr-3 w-100">
